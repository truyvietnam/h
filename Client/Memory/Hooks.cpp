	...
	// Vtables
	{
		...
		// LocalPlayer::vtable
		{
			...
			if (localPlayerVtable == 0x0 || sigOffset == 0x0)
				...
				//g_Hooks.Actor_startSwimmingHook = std::make_unique<FuncHook>(localPlayerVtable[201], Hooks::Actor_startSwimming);DF
...
__int64 Hooks::RenderText(__int64 a1, C_MinecraftUIRenderContext* renderCtx) {
		...
		if (strcmp(screenName.c_str(), "start_screen") == 0) {
...

			{
				...
				if (moduleMgr->isInitialized() && shouldRenderArrayList) {
					...
					float textPadding = 2.0f * textSize;
					float textHeight = 5.0f * textSize;
					float smoothness = 2;
	...
	if (notificationsMod->isEnabled()) {
		...
		for (auto& notification : notifications) {
			notification->fade();
			if (notification->fadeTarget == 1 && notification->duration <= 0 && notification->duration > -0.5)
				notification->fadeTarget = 0;
			else if (notification->duration > 0 && notification->fadeVal > 0.6f)
